...
class Logger:
    ...
    def write(self, json_text):
        ...
        if lKeyC >= self.log_level or self.log_level == 5:
            ...
            newtext = json.dumps(json_text["value"])
            ...
            try:
                if json_text["type"] == "MAINCORE":
                    self.currenttext_core = self.currenttext_core + newtext
                    if getsizeof(self.currenttext_core) > 800 or self.bulk == 0 or self.log_level == 5:
                        errorlog = open("{0}/PeopleCounting.log".format(self.logs_dir), "ab")
                        errorlog.write(self.currenttext_core.encode('utf-8'))
                        self.currenttext_core = ""
                        errorlog.close()
